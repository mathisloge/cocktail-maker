...
boost::asio::awaitable<void> Drv8825StepperMotorDriver::step(units::Steps steps,
                                                             units::StepsPerSecond velocity)
{
    auto exec = co_await async::this_coro::executor;
    async::steady_timer timer{exec};

    const auto direction =
        steps > 0 * units::step ? gpiod::line::value::ACTIVE : gpiod::line::value::INACTIVE;
    ...
    const auto steps_double =
        (0.5 * kAcceleration * mp_units::pow<2>(ramp_time)).numerical_value_in(units::step);
    ...
    for (units::Steps i = 1 * units::step; i <= ramp_steps; i += 1 * units::step) {
        ...
        co_await step_one(timer,
                          std::chrono::duration_cast<std::chrono::microseconds>(
                              units::to_chrono_duration(delay)));
    }
    ...
    for (units::Steps i = 0 * units::step; i < cruise_steps; i += 1 * units::step) {
        co_await step_one(timer,
                          std::chrono::duration_cast<std::chrono::microseconds>(
                              mp_units::to_chrono_duration(cruise_delay)));
    }
    ...
    for (units::Steps i = ramp_steps; i >= 1 * units::step; i -= 1 * units::step) {
        ...
        co_await step_one(timer,
                          std::chrono::duration_cast<std::chrono::microseconds>(
                              mp_units::to_chrono_duration(delay)));
    }
}

boost::asio::awaitable<void> Drv8825StepperMotorDriver::step_one(
    boost::asio::steady_timer& timer, std::chrono::microseconds wait_after)
{
